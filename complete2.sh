...
function VC_INSTALL() {
  ...
  if [[ $(lspci -k | grep VGA | grep -i amd) ]]; then
    ...
    sleep 2
    sudo pacman -S --noconfirm --needed opencl-mesa lib32-opencl-mesa vulkan-mesa-layers lib32-vulkan-mesa-layers mesa-vdpau lib32-mesa-vdpau vulkan-radeon lib32-vulkan-radeon ocl-icd lib32-ocl-icd intel-compute-runtime intel-graphica-compiler intel-opencl-clang vulkan-icd-loader lib32-vulkan-icd-loader vk3d lib32-vk3d  vulkan-swrast  # Testing stuff amdvlk lib32-amdvlk
